...
class TensorBox(object):
    def __init__(self, H):
        for key, value in list(H.items()):
            # Make sure the integers are integers so tensorflow doesn't throw a bloody fit
            try:
                val_int = int(value)
                if value == val_int:
                    td[key]=val_int
    ...
    def build_forward(self, x, phase, reuse):
        ...
        with tf.variable_scope('decoder', reuse=reuse, initializer=initializer):
            ...
            if self.H['use_rezoom']:
                ...
                for k in range(self.H['rnn_len']):
                    delta_features = tf_concat(1, [lstm_outputs[k], old_div(rezoom_features[:, k, :], 1000.)])
                    dim = 128
        ...
        for phase in ['train', 'test']:
            ...
            if phase == 'test':
                ...
                def log_image(np_img, np_confidences, np_boxes, np_global_step, pred_or_true):
                    ...
                    num_images = 10
                    img_path = os.path.join(self.H['save_dir'], '%s_%s.jpg' % ((old_div(np_global_step, self.H['logging']['display_iter'])) % num_images, pred_or_true))
        ...
        with tf.Session(config=config) as sess:
            ...
            for i in range(max_iter):
                ...
                adjusted_lr = (self.H['solver']['learning_rate'] *
                               0.5 ** max(0, (old_div(i, self.H['solver']['learning_rate_step'])) - 2))
                ...
                if i % display_iter != 0:
                    ...
                    if i > 0:
                        dt = old_div((time.time() - start), (self.H['batch_size'] * display_iter))
                    start = time.time()
